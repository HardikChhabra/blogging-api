...
export async function readCommentsByBlog (req: Request, res: Response) {
    try{
        ...
        const blogComments = await db.select().from(comments).where(eq(comments.blogId, blogId));
...
export async function updateComment (req: Request, res: Response) {
    try{
        ...
        const [updatedComment] = await db.update(comments).set(updatedFeilds).where(eq(comments.commentId, req.params.id));
        if(!updatedComment){
